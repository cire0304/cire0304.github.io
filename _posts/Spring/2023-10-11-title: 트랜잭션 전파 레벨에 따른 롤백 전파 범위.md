...
또한 `@Transaction` 어노테이션을 중첩 사용하여, **외부 트랜잭션**과 **내부 트랜잭션**으로 트랜잭션 범위를 설정할 수 있습니다.

이때, **트랜잭션 전파 정책**에 따라 **내부 트랜잭션**에서 `예외`가 발생했을 때, **외부 트랜잭션** 의 롤백 여부를 결정할 수 있습니다.

본 포스팅에서는 여러 케이스를 만들어, 테스트후 언제 **외부 트랜잭션**이 롤백되는지 확인하려합니다.
...
- (CASE 1) 외부 tx -> 내부 default tx -> 내부 UncheckedException 예외 -> 내부 예외처리
- (CASE 2) 외부 tx -> 내부 default tx -> 내부 UncheckedException 예외 -> 외부 예외처리
- (CASE 3) 외부 tx -> 내부 required_new tx -> 내부 UncheckedException 예외 -> 내부 예외처리
- (CASE 4) 외부 tx -> 내부 required_new tx -> 내부 UncheckedException 예외 -> 외부 예외처리

### CheckedException

- (CASE 5) 외부 tx -> 내부 default tx -> 내부 CheckedException 예외 -> 외부 예외처리
...
## (CASE 1) 외부 tx -> 내부 default tx -> 내부 예외 -> 내부 예외처리
...
**내부 트랜잭션**에서 예외를 잡는 케이스입니다. 이때 저장되는 `Coupon`은 2개입니다. 예외를 **내부 트랜잭션**에서 잡았기 때문에 롤백 전파가 되지 않습니다.


## (CASE 2) 외부 tx -> 내부 default tx -> 내부 예외 -> 외부 예외처리
...
`Coupon`이 하나도 저장되지 않은 이유는 **내부 트랜잭션**이 실패함에 따라 아래의 메시지와 함께 **외부 트랜잭션**이 롤백되었기 때문입니다.

`...UnexpectedRollbackException: Transaction silently rolled back because it has been marked as rollback-only`

아래의 로그를 보면 왜 외부 트랜잭션이 실패했는지 알 수 있습니다.
...
## (CASE 3) 외부 tx -> 내부 required_new tx -> 내부 예외 -> 내부 예외처리

(CASE 1)과 마찬가지로, 예외를 **내부 트랜잭션**에서 잡았기 때문에, 저장되는 `Coupon`은 2개 입니다.

## (CASE 4) 외부 tx -> 내부 required_new tx -> 내부 예외 -> 외부 예외처리
...
...jpa.JpaTransactionManager         | : Found thread-bound EntityManager [SessionImpl(1561240239<open>)] for JPA transaction
...
...jpa.JpaTransactionManager        <5>: Opened new EntityManager [SessionImpl(1124119884<open>)] for JPA transaction
...internal.TransactionImpl          | : On TransactionImpl creation, JpaCompliance#isJpaTransactionComplianceEnabled == false
...internal.TransactionImpl          | : begin
...
...jpa.JpaTransactionManager        <6>: Resuming suspended transaction after completion of inner transaction
...
- <4> : **내부 트랜잭션**이 끝났기 때문에 <2>에서 보류했던 트랜잭션을 다시 시작합니다.
- <5> : 또 다시 **트랜잭션 전파 정책**이 `Requried_new`인 메서드(fail())를 만들었기 때문에, 현재의 트랜잭션을 잠시 보류합니다.
- <5> : `SaveCouponServiceCase4.fail`에서 예외가 발생했기 때문에, 롤백됩니다.
- <6> : **내부 트랜잭션**이 롤백되었지만, **내부 트랜잭션**과 **외부 트랜잭션**은 별개의 트랜잭션이기 때문에 커밋됩니다.

## (CASE 5) 외부 tx -> 내부 default tx -> 내부 예외 -> 외부 예외처리

`CASE 2`에서 내부에서 `UncheckedExecption`예외가 발생하고, 외부에서 예외처리를 했을 때, **전역 롤백**되는 것을 확인했습니다.
...
- <4> : 따라서, **외부 트랜잭션**이 커밋되는 것을 확인할 수 있습니다.
...
이상 어느 경우 **외부 트랜잭션**이 롤백하는지에 대해 알아보았습니다.
끝까지 봐주셔서 감사합니다!
