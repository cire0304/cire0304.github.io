...
![](../../image/Toy-Project/[토이%20프로젝트]%20초기%20응답%20지현%20이슈%20해결%20:%20warm%20up/쿠폰%20조회%20초기%20응답%20지연%20형상%20-%204.png)
...
남은 재고가 1 이상인 쿠폰이면 업데이트하는 네이티브 쿼리를 사용하여 쿠폰 재고 감소 로직을 수행하였습니다.
그리고 네이티브 쿼리 결과에 따라 쿠폰 발급 성공 여부를 판단합니다. (위 코드의 네이티브 쿼리 반환값은 쿼리에 의해 영향을 받은 레코드 수입니다.)

그 결과 `Lock`으로 인한 성능 저하를 최소화 하였고, 성능(TPS)을 약 36% 향상 시킬 수 있었습니다.
